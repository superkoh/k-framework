...
import me.superkoh.kframework.lib.payment.wechat.sdk.service.ReverseService;
import me.superkoh.kframework.lib.payment.wechat.sdk.service.ScanPayQueryService;
import me.superkoh.kframework.lib.payment.wechat.sdk.service.ScanPayService;
...
public class ScanPayBusiness {
    ...
    public ScanPayBusiness() throws IllegalAccessException, ClassNotFoundException, InstantiationException, IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        scanPayService = new ScanPayService();
        scanPayQueryService = new ScanPayQueryService(null);
    ...
    public interface ResultListener {
        ...
        void onFailByReturnCodeError(WXPayData scanPayResData);

        //API返回ReturnCode为FAIL，支付API系统返回失败，请检测Post给API的数据是否规范合法
        void onFailByReturnCodeFail(WXPayData scanPayResData);

        //支付请求API返回的数据签名验证失败，有可能数据被篡改了
        void onFailBySignInvalid(WXPayData scanPayResData);


        //用户用来支付的二维码已经过期，提示收银员重新扫一下用户微信“刷卡”里面的二维码
        void onFailByAuthCodeExpire(WXPayData scanPayResData);

        //授权码无效，提示用户刷新一维码/二维码，之后重新扫码支付"
        void onFailByAuthCodeInvalid(WXPayData scanPayResData);

        //用户余额不足，换其他卡支付或是用现金支付
        void onFailByMoneyNotEnough(WXPayData scanPayResData);

        //支付失败
        void onFail(WXPayData scanPayResData);

        //支付成功
        void onSuccess(WXPayData scanPayResData);
     ...
     * @throws Exception
     */
    public void run(WXPayData scanPayReqData, ResultListener resultListener) throws Exception {

        //--------------------------------------------------------------------
        //构造请求“被扫支付API”所需要提交的数据
        //--------------------------------------------------------------------

        String outTradeNo = (String)scanPayReqData.getValue(WXPayConstants.outTradeNoKey);
        ...
        WXPayData scanPayResData = new WXPayData();
        scanPayResData.fromXml(payServiceResponseString);
        //异步发送统计请求
        //*

        ReportReqData reportReqData = new ReportReqData(
                null,
                (String)scanPayReqData.getValue(WXPayConstants.deviceInfoKey),
                Configure.PAY_API,
                (int) (totalTimeCost),//本次请求耗时
                (String)scanPayResData.getValue(WXPayConstants.returnCodeKey),
                (String)scanPayResData.getValue(WXPayConstants.returnMsgKey),
                (String)scanPayResData.getValue(WXPayConstants.resultCodeKey),
                (String)scanPayResData.getValue(WXPayConstants.errCodeKey),
                (String)scanPayResData.getValue(WXPayConstants.errCodeDesKey),
                (String)scanPayResData.getValue(WXPayConstants.outTradeNoKey),
                (String)scanPayReqData.getValue(WXPayConstants.spbillCreateIpKey)
        );
        ...
        if (scanPayResData.getValue(WXPayConstants.returnCodeKey) == null) {
            log.e("【支付失败】支付请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问");
            resultListener.onFailByReturnCodeError(scanPayResData);
            return;
        }

        if (scanPayResData.getValue(WXPayConstants.returnCodeKey).equals("FAIL")) {
            ...
            //--------------------------------------------------------------------
            if (!Signature.checkIsSignValidFromResponseString(payServiceResponseString,null)) {
            ...
            //获取错误码
            String errorCode = (String)scanPayResData.getValue(WXPayConstants.errCodeKey);
            //获取错误描述
            String errorCodeDes = (String)scanPayResData.getValue(WXPayConstants.errCodeDesKey);

            if (scanPayResData.getValue(WXPayConstants.resultCodeKey).equals("SUCCESS")) {
    ...
    private boolean doOnePayQuery(String outTradeNo) throws Exception {
        ...
        WXPayData scanPayQueryReqData = WXPayData.payQueryReqData(null, "", outTradeNo);
        ...
        //将从API返回的XML数据映射到Java对象
        WXPayData payQueryResData = new WXPayData();
        payQueryResData.fromXml(payQueryServiceResponseString);
        if (payQueryResData.getValue(WXPayConstants.returnCodeKey) == null) {
            log.i("支付订单查询请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }

        if (payQueryResData.getValue(WXPayConstants.returnCodeKey).equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            log.i("支付订单查询API系统返回失败，失败信息为：" + payQueryResData.getValue(WXPayConstants.returnMsgKey));
            return false;
        } else {
            if (payQueryResData.getValue(WXPayConstants.resultCodeKey).equals("SUCCESS")) {//业务层成功
                if (payQueryResData.getValue(WXPayConstants.tradeStateKey).equals("SUCCESS")) {
                    //表示查单结果为“支付成功”
                    log.i("查询到订单支付成功");
                    return true;
                } else {
                ...
                log.i("查询出错，错误码：" + payQueryResData.getValue(WXPayConstants.errCodeKey) + "     错误信息：" +
                        payQueryResData.getValue(WXPayConstants.errCodeDesKey));
                return false;
            }
        }
    }
    ...
    private boolean doOneReverse(String outTradeNo) throws Exception {
        ...
        WXPayData reverseReqData = WXPayData.revokeReqData(null, "", outTradeNo);
        reverseResponseString = reverseService.request(reverseReqData);

        log.i("撤销API返回的数据如下：");
        log.i(reverseResponseString);
        //将从API返回的XML数据映射到Java对象
        WXPayData reverseResData = new WXPayData();
        reverseResData.fromXml(reverseResponseString);
        if (reverseResData.getValue(WXPayConstants.returnCodeKey) == null) {
            log.i("支付订单撤销请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }
        if (reverseResData.getValue(WXPayConstants.returnCodeKey).equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            log.i("支付订单撤销API系统返回失败，失败信息为：" + reverseResData.getValue(WXPayConstants.returnMsgKey));
            return false;
        } else {
            if (reverseResData.getValue(WXPayConstants.resultCodeKey).equals("FAIL")) {
                log.i("撤销出错，错误码：" + reverseResData.getValue(WXPayConstants.errCodeKey) + "     错误信息：" +
                        reverseResData.getValue(WXPayConstants.errCodeDesKey));
                if (reverseResData.getValue(WXPayConstants.recallKey).equals("Y")) {
                    //表示需要重试
