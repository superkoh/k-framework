...
import me.superkoh.kframework.lib.payment.wechat.sdk.service.RefundService;
...
public class RefundBusiness {
    ...
    public RefundBusiness() throws IllegalAccessException, ClassNotFoundException, InstantiationException, IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        refundService = new RefundService(null);
    }

    public interface ResultListener{
        //API返回ReturnCode不合法，支付请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问
        void onFailByReturnCodeError(WXPayData refundResData);

        //API返回ReturnCode为FAIL，支付API系统返回失败，请检测Post给API的数据是否规范合法
        void onFailByReturnCodeFail(WXPayData refundResData);

        //支付请求API返回的数据签名验证失败，有可能数据被篡改了
        void onFailBySignInvalid(WXPayData refundResData);

        //退款失败
        void onRefundFail(WXPayData refundResData);

        //退款成功
        void onRefundSuccess(WXPayData refundResData);
     ...
     * @throws Exception
     */
    public void run(WXPayData refundReqData,ResultListener resultListener) throws Exception {
        ...
        WXPayData refundResData = new WXPayData();
        refundResData.fromXml(refundServiceResponseString);

        ReportReqData reportReqData = new ReportReqData(
                null,
                (String)refundResData.getValue(WXPayConstants.deviceInfoKey),
                Configure.REFUND_API,
                (int) (totalTimeCost),//本次请求耗时
                (String)refundResData.getValue(WXPayConstants.returnCodeKey),
                (String)refundResData.getValue(WXPayConstants.returnMsgKey),
                (String)refundResData.getValue(WXPayConstants.resultCodeKey),
                (String)refundResData.getValue(WXPayConstants.errCodeKey),
                (String)refundResData.getValue(WXPayConstants.errCodeDesKey),
                (String)refundResData.getValue(WXPayConstants.outRefundNoKey),
                Configure.getIP()
        );
        ...
        if (refundResData.getValue(WXPayConstants.returnCodeKey) == null) {
        ...
        if (refundResData.getValue(WXPayConstants.returnCodeKey).equals("FAIL")) {
            ...
            if (!Signature.checkIsSignValidFromResponseString(refundServiceResponseString, null)) {
                setResult("Case3:退款请求API返回的数据签名验证失败，有可能数据被篡改了",Log.LOG_TYPE_ERROR);
                resultListener.onFailBySignInvalid(refundResData);
                return;
            }

            if (refundResData.getValue(WXPayConstants.resultCodeKey).equals("FAIL")) {
                log.i("出错，错误码：" + refundResData.getValue(WXPayConstants.errCodeKey) + "     错误信息："
                        + refundResData.getValue(WXPayConstants.errCodeDesKey));
