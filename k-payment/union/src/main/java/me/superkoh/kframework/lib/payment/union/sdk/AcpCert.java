...
//	private static X509Certificate encryptTrackCert = null;
	 ...
	 * 初始化所有证书.
	 */
	public AcpCert(SDKConfig config) {
		this.config = config;
		addProvider();
...
//		if (!SDKUtil.isEmpty(config.getEncryptTrackKeyModulus())
//				&& !SDKUtil.isEmpty(config.getEncryptTrackKeyExponent())) {
...
//		}
	...
	public PrivateKey getSignCertPrivateKey() {
		try {
			...
			PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias,
					this.config.getSignCertPwd().toCharArray());
			return privateKey;
	...
	public PublicKey getEncryptCertPublicKey() {
		if (null == encryptCert) {
			String path = this.config.getEncryptCertPath();
