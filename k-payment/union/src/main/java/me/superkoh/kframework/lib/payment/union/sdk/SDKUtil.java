...
public class SDKUtil {
	 ...
	 * @return 签名是否成功
	 */
	public static boolean sign(Map<String, String> data, String encoding, SDKConfig config) {
		if (isEmpty(encoding)) {
			encoding = "UTF-8";
		}
		// 设置签名证书序列号
		data.put(SDKConstants.param_certId, AcpCertManager.getCertByConfig(config).getSignCertId());
		...
		try {
			...
			byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
					AcpCertManager.getCertByConfig(config).getSignCertPrivateKey(), signDigest));
	...
	public static boolean signByCertInfo(Map<String, String> data,
			 String certPath, String certPwd,String encoding, SDKConfig config) {
		if (isEmpty(encoding)) {
			encoding = "UTF-8";
		}
		...
		data.put(SDKConstants.param_certId,
				AcpCertManager.getCertByConfig(config).getCertIdByKeyStoreMap(certPath, certPwd));
		// 将Map信息转换成key1=value1&key2=value2的形式
		String stringData = coverMap2String(data);
		/**
		 * 签名\base64编码
		...
		try {
			byte[] signDigest = SecureUtil.sha1X16(stringData, encoding);
			byteSign = SecureUtil.base64Encode(SecureUtil.signBySoft(
					AcpCertManager.getCertByConfig(config).getSignCertPrivateKeyByStoreMap(certPath, certPwd),
					signDigest));
			stringSign = new String(byteSign);
			// 设置签名域值
			data.put(SDKConstants.param_signature, stringSign);
